...
def evaluate(model, tune_x_data):
    ...
    print "the average MAP score for test text is: ", avg_mean_reciprocal_rank
        ...
        for x_key in x_train:
            ...
            for x_i in x_list:
                ...
                if i % batch_size == 0:
                    ...
                    if len(x) == 0:
                        break
                    model.sdg_step(x, y, u, learning_rate, len(x))
                    # print model.forward(x, y, u)
                    loss += model.cal_loss_function(x, y, u)
...
print datetime.datetime.now()
# read train data
train_file_path = '../foursquare/train.txt'
test_file_path = '../foursquare/test.txt'
x_data = read_foursquare_data(train_file_path)
tune_x_data = read_foursquare_data(test_file_path)
...
if not _TRAINED_MODEL_:
    ...
    model = LSTM_neg(hidden_dim=_HIDDEN_DIM_, num_user=num_user, num_poi=num_poi)
else:
    # trained with existed model
    model = load_model_parameters_lstm1(path=_TRAINED_MODEL_)
